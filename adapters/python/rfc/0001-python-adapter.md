...
DSC engine startup
    ...
    │       • Scans importlib.metadata distributions for *.dsc.adaptedResource.json
    │       • Emits {"manifestPath": "<abs_path>"} per manifest found
    │       • Results cached in ~/.dsc/PythonDiscoverCache.json
    │       • Cache keyed by installed-distribution fingerprint (name==version pairs)
    │
...
Pre-built manifests (fast, preferred) are generated by `dsc-gen manifest` or by
the Hatchling build hook at wheel-build time.  The `list` fallback ensures that
resources installed in development mode (editable installs without a manifest
build step) are still discoverable.
...
Both the adapter and the discovery extension use a JSON cache keyed by a
fingerprint of all installed distributions (`name==version|...` sorted).  When
the fingerprint changes (package installed/upgraded/removed), the cache is
automatically invalidated and rebuilt on the next invocation.  An additional
path-existence check ensures stale entries from removed packages are evicted.

Cache locations:
- Windows: `%LOCALAPPDATA%\.dsc\Python{Discover,List}Cache.json`
- Linux/macOS: `~/.dsc/Python{Discover,List}Cache.json`

---

## Alternatives considered

### Alternative A: Ship `ms-dsc` inside the DSC distribution

Bundling `ms-dsc` with DSC would guarantee the adapter and SDK are always in sync.
Rejected because:
- PyPI distribution is the standard mechanism for Python packages.
- Resource authors update `ms-dsc` independently of DSC.
- Bundling creates a version coupling problem.

### Alternative B: Single Python file adapter (no package structure)
...
### Alternative C: Require Pydantic for all resources
...
### Alternative D: Use importlib.resources for manifest location

`importlib.resources` could locate manifest files without scanning the filesystem.
Rejected because:
- It requires resources to be declared explicitly (not auto-discovered).
- Editable installs complicate `importlib.resources` access.
- The current filesystem scan + cache approach is more robust across install modes.

---

## Security considerations

- The adapter executes **no shell commands** — all subprocess invocations are
  initiated by the DSC engine, not the adapter.
- `importlib.metadata.entry_points` is used for class resolution, not `eval` or
  `exec`.  A malicious package could register a hostile entry point, but this
  requires installation-level access (not a vulnerability specific to this adapter).
- The cache files are written to user-local directories with standard file
  permissions.  No sensitive data is cached — only package names, versions, and
  file paths.
- Input JSON is parsed with `json.loads` (no `eval`) and unknown fields are
  silently discarded during schema instantiation.

---

## Open questions

1. **`_exist` naming convention** — DSC v3 uses `_exist` as the canonical presence
   property.  Should the SDK provide a mixin or marker that makes this convention
   explicit and discoverable? Currently it's documented but not enforced.

2. **Pydantic v1 support** — `PydanticSchemaProvider` uses `model_json_schema()`
   (Pydantic v2 API).  Should we support Pydantic v1 as well, or declare v2 the
   minimum?

3. **Windows-only resources** — Resources targeting Windows-specific APIs can
   guard with `sys.platform == "win32"` but the adapter itself is cross-platform.
   Should the adapter manifest include a `condition` field to skip Windows-only
   resources on other platforms?

4. **`dsc-gen validate` command** — should `ms_dsc.cli` include a `dsc-gen validate`
   sub-command that imports a resource class and reports capability/schema issues
   without writing files?  This would be useful as a pre-commit check.

5. **Async resources** — should the adapter support `async def get()` etc. with an
   internal `asyncio.run()` wrapper, enabling resources that perform I/O-bound work?
...
| Area | Status |
|------|--------|
| Adapter runtime (`pyadapter/`) | ✅ Complete |
| SDK (`ms-dsc`) | ✅ Complete |
| Discovery extension | ✅ Complete |
| Hatchling build hook | ✅ Complete |
| Unit tests (pytest) | ✅ 160+ tests |
| Integration tests (Pester) | ✅ Component + DSC CLI layers |
| Example: `file_presence` | ✅ All 5 capabilities |
| Example: `dsc-example-resource` | ✅ Greeting resource (installable demo) |
| RFC | This document |
